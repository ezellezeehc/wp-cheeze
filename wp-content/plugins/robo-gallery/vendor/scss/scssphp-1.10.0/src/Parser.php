...
final class Parser
{
        ...
        if (isset($this->buffer[$this->count]) && $this->buffer[$this->count] === '@') {
            ...
            if (
                ...
                if (! isset($this->charset)) {
                    ...
                    [$line, $column] = $this->getSourcePosition($s);
    ...
    private function unit(&$unit): bool
    {
        ...
        if ($this->match('([0-9]*(\.)?[0-9]+)([%a-zA-Z]+)?', $m, false)) {
            if (\strlen($this->buffer) === $this->count || ! $this->sm_ctype_digit($this->buffer[$this->count])) {
                $this->whitespace();
    ...
    /**
     * Helper method to check string
     *
     * @param string 
     */
    private function sm_ctype_space($string) {
    ...
    private function sm_ctype_digit( $string ) {
